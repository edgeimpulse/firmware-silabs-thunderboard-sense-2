...
#
#	Generated from com.silabs.ss.framework.project.api.toolchain.core.scripting.ISLSStockConfigsModule 
#	API version 0
#

class stockConfig(com_silabs_ss_platform_api_scripting_core_ISLSManagerModuleBase):
  '''
  This module provides access to the Studio stock configurations (aka targets)
   *
  '''
  NAME = str
  '''
  ...
  def isCompatibleWithPart(self, stockConfig, part):
    '''
    Checks if this stockConfig is compatible with the given part
    @param stockConfig a str, Descriptor, or Entity to match against the part
    @param part a str, Descriptor, or Entity to match against the stockConfig
    @type stockConfig: object
    @type part: object
    @return True if the stockConfig and part are compatible, False otherwise
    '''
    return bool()


  def isCompatibleWithToolchain(self, stockConfig, toolchain):
    '''
    Checks if this stockConfig is compatible with the given toolchain
    @param stockConfig a str, Descriptor, or Entity to match against the toolchain
    @param toolchain a str, Descriptor, or Entity to match against the stockConfig
    @type stockConfig: object
    @type toolchain: object
    @return True if the stockConfig and toolchain are compatible, False otherwise
    '''
    return bool()
