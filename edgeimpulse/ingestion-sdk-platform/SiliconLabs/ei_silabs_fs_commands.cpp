...
/** Silicon Labs MAX25flash_spi return messages */
typedef enum {
 ...
 *
 * @return     ei_silabs_ret_t enum
 */
int ei_silabs_fs_load_config(uint32_t *config, uint32_t config_size)
{
	int retVal = SILABS_FS_CMD_OK;

	if(config == NULL) {
		return SILABS_FS_CMD_NULL_POINTER;
	}
	...
	if(flash_wait_while_busy() == 0) {
		retVal = SILABS_FS_CMD_READ_ERROR;
	}
	else {
...
int ei_silabs_fs_save_config(const uint32_t *config, uint32_t config_size)
{
	...
	return (retVal == SILABS_FS_CMD_OK) ?
...
int ei_silabs_fs_write_samples(const void *sample_buffer, uint32_t address_offset, uint32_t n_samples)
{
	...
	if((address_offset + n_word_samples) > SIZE_RAM_BUFFER) {
		return SILABS_FS_CMD_WRITE_ERROR;
	}
...
int ei_silabs_fs_read_sample_data(void *sample_buffer, uint32_t address_offset, uint32_t n_read_bytes)
{
	...
	if((address_offset + n_read_bytes) > SIZE_RAM_BUFFER) {
		return SILABS_FS_CMD_READ_ERROR;
	}
	else if(sample_buffer == 0) {
		return SILABS_FS_CMD_NULL_POINTER;
	}
...
static uint32_t flash_erase_sectors(uint32_t startAddress, uint32_t nSectors)
{
	...
	do {
		if(flash_wait_while_busy() == 0) {
			return SILABS_FS_CMD_ERASE_ERROR;
		}
...
static void flash_program_page(uint32_t byteAddress, uint8_t *page, uint32_t pageBytes)
{
	MX25_PP(byteAddress, page, pageBytes);
}
...
static uint32_t flash_read_data(uint32_t byteAddress, uint8_t *buffer, uint32_t readBytes)
{
    MX25_READ(byteAddress, buffer, readBytes);
