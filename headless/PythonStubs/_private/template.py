...
#
#	Generated from com.silabs.ss.framework.project.api.templates.scripting.ISLSTemplateModule 
#	API version 0
#

class template(com_silabs_ss_platform_api_scripting_core_ISLSManagerModuleBase):
  '''
  This module provides access to the Studio templates
   *
  '''
  INITIALLY_OPENED_RESOURCE = str
  '''
  Value of initially opened resource for use with template properties
  @value TemplateProperties.INITIALLY_OPENED_RESOURCE
  '''
  ...
  def isCompatibleWithBoard(self, template, board):
    '''
    Checks if this template is compatible with the given board
    @param template a str, Descriptor, or Entity to match against the board
    @param board a str, Descriptor, or Entity to match against the template
    @type template: object
    @type board: object
    @return True if the template and board are compatible, False otherwise
    '''
    return bool()


  def isCompatibleWithPart(self, template, part):
    '''
    Checks if this template is compatible with the given part
    @param template a str, Descriptor, or Entity to match against the part
    @param part a str, Descriptor, or Entity to match against the template
    @type template: object
    @type part: object
    @return True if the template and part are compatible, False otherwise
    '''
    return bool()


  def isCompatibleWithSDK(self, template, sdk):
    '''
    Checks if this template is compatible with the given sdk
    @param template a str, Descriptor, or Entity to match against the sdk
    @param sdk a str, Descriptor, or Entity to match against the template
    @type template: object
    @type sdk: object
    @return True if the template and sdk are compatible, False otherwise
    '''
    return bool()


  def isCompatibleWithToolchain(self, template, toolchain):
    '''
    Checks if this template is compatible with the given toolchain
    @param template a str, Descriptor, or Entity to match against the toolchain
    @param toolchain a str, Descriptor, or Entity to match against the template
    @type template: object
    @type toolchain: object
    @return True if the template and toolchain are compatible, False otherwise
    '''
    return bool()
  ...
  def findAllCompatibleTemplates(self, board, part, sdk, toolchain):
    '''
    Get a list of all templates that are compatible with the input variables. 
    If any input it None, it will default to ALL compatibility.
    @param board a str, Descriptor, or Entity to match against the templates
    @param part a str, Descriptor, or Entity to match against the templates
    @param sdk a str, Descriptor, or Entity to match against the templates
    @param toolchain a str, Descriptor, or Entity to match against the templates
    @type board: object
    @type part: object
    @type sdk: object
    @type toolchain: object
    @return a list of templates that will work with the supplied entities
    '''
    return list()
