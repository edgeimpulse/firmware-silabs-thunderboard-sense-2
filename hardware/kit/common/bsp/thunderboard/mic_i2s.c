...
static volatile int  sampleBufferReady;    /**< Flag to show that the buffer is filled with new samples   */
static int           sampleBufSelect;
 ...
 *
 * @param[in] buffer
 *    Pointer to the sample buffer to store the ADC data. Should be twice
 *    the size of len!
 *
 * @param[in] len
...
/***************************************************************************//**
 * @brief
 *    Checks if the I2S DMA is busy
 *
 * @return
 *    Returns number of sample buffer ready, if busy 0
 ******************************************************************************/
int MIC_isBusy(void)
{
  int retVal = 0;

  if(sampleBufferReady != 0) {
    retVal = sampleBufferReady;
...
static bool dmaCompleteCallback(unsigned int channel, unsigned int sequenceNo, void *userParam)
{
  ...
  if ( sampleIndex >= sampleCount ) {
    ...
    sampleBufSelect ^= 1;
    sampleBufferReady = sampleBufSelect + 1;
  }

  
  nextDesc->xfer.dstInc  = ldmaCtrlDstIncOne;
  nextDesc->xfer.dstAddr = (uint32_t) &sampleBuffer[(sampleBufSelect * sampleBufferLen) + sampleIndex];
