...
#
#	Generated from com.silabs.ide.kit.core.scripting.ISLSKitModule 
#	API version 0
#

class kit(com_silabs_ide_internal_api_scripting_core_ISLSManagerModuleBase):
  '''
  This module provides access to Studio kits.
   *
  '''
  NAME = str
  '''
  ...
  def isCompatibleWithKit(self, kit, matchedKit):
    '''
    Check if the two kits are compatible with each other
    @param kit a str, Descriptor, or Entity for the kit
    @param matchedKit a str, Descriptor, or Entity to match with the other kit
    @type kit: object
    @type matchedKit: object
    @return True if the two kits are compatible, False otherwise
    '''
    return bool()


  def isCompatibleWithPart(self, kit, matchedPart):
    '''
    Checks if the two parts are compatible with each other
    @param kit a str, Descriptor, or Entity for the kit
    @param matchedPart a str, Descriptor, or Entity to match with the other part
    @type kit: object
    @type matchedPart: object
    @return True if the two parts are compatible, False otherwise
    '''
    return bool()
  ...
  def detectAndEnableEntity(self, path, options, monitor):
    ...
    '''
    return com_silabs_ide_internal_api_core_manager_IDescriptorEntity()
  ...
  def getManager(self):
    ...
    '''
    return com_silabs_ide_internal_api_core_manager_IManager()
