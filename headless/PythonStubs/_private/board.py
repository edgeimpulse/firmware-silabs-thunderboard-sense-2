...
#
#	Generated from com.silabs.ss.platform.api.content.board.core.scripting.ISLSBoardModule 
#	API version 0
#

class board(com_silabs_ss_platform_api_scripting_core_ISLSManagerModuleBase):
  '''
  This module provides access to Studio boards.
   *
  '''
  NAME = str
  '''
  ...
  def isCompatibleWithBoard(self, board, matchedBoard):
    '''
    Check if the two boards are compatible with each other
    @param board a str, Descriptor, or Entity for the board
    @param matchedBoard a str, Descriptor, or Entity to match with the other board
    @type board: object
    @type matchedBoard: object
    @return True if the two boards are compatible, False otherwise
    '''
    return bool()


  def isCompatibleWithPart(self, board, matchedPart):
    '''
