...
#
#	Generated from com.silabs.ss.framework.project.api.toolchain.core.scripting.ISLSToolchainModule 
#	API version 0
#

class toolchain(com_silabs_ss_platform_api_scripting_core_ISLSManagerModuleBase):
  '''
  This module provides access to the Studio toolchains
   *
  '''
  NAME = str
  '''
  ...
  def isCompatibleWithPart(self, toolchain, part):
    '''
    Checks if this toolchain is compatible with the given part
    @param toolchain a str, Descriptor, or Entity to match against the part
    @param part a str, Descriptor, or Entity to match against the toolchain
    @type toolchain: object
    @type part: object
    @return True if the toolchain and part are compatible, False otherwise
    '''
    return bool()


  def isCompatibleWithToolchain(self, toolchain, matchedToolchain):
    '''
    Check if the two toolchains are compatible with each other
    @param toolchain a str, Descriptor, or Entity to match with the other toolchain
    @param matchedToolchain a str, Descriptor, or Entity to match with the other toolchain
    @type toolchain: object
    @type matchedToolchain: object
    @return True if the two toolchains are compatible, False otherwise
    '''
    return bool()
